...
class DialogOptions extends DialogExtension
{
	...
	public var selectionWindow:DialogWindow;
	...
	public function option(args:Array<Array<Dynamic>>):Void
	{
		...
		if(useImage)
			selectionImg = style.optCursorImage.clone();
		else
		...
		if(style.optChoiceLayout == "Horizontal")
		{
			...
			h = f.info.scaledLineHeight;
		}
		else
		{
			h -= G2.s(style.optItemPadding);
		}
